...
RUN chmod +x /root/setup
    RUN mysql -u root -proot -e "CREATE DATABASE IF NOT EXISTS Jiradb DEFAULT CHARACTER SET utf8 COLLATE utf8_bin;"
    RUN mysql -u root -proot -e "use Jiradb;"
    RUN mysql -u root -proot -e "CREATE USER '${MYSQL_USER}' @ 'localhost' IDENTIFIED BY '${MYSQL_PASS}';"
    RUN mysql -u root -proot -e "GRANT ALL PRIVILEGES ON *.* TO '${MYSQL_USER}'@'localhost' WITH GRANT OPTION; FLUSH PRIVILEGES;"
